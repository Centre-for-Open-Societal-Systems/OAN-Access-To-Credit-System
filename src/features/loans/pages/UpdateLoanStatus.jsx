...
import { DUMMY_LOANS } from '../data/loans.mock.js';
import {
  STATUS_CFG,
  LOAN_STATUSES,
  UPDATABLE_STATUSES,
...
} from '../constants/loans.constants.js';
...
// ─── Update Panel ─────────────────────────────────────────────────────────────
function UpdatePanel({ loan, onClose, onConfirm }) {
              ...
              {newStatus && (
                <section>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(STATUS_UPDATE_REASONS[newStatus] ?? []).map((r) => (
                      <button
                        key={r}
                        type="button"
                        onClick={() => setReason(r)}
...
function UpdateLoanStatus() {
  ...
  const [panelLoan,  setPanelLoan]  = useState(null);
  ...
  function handleStatusUpdate({ loanId, newStatus }) {
    const now     = new Date();
    const updated = `${now.getDate()} ${now.toLocaleString('default', { month: 'short' })} ${now.getFullYear()}, ${now.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}`;
    setLoans((prev) =>
      prev.map((l) =>
        l.id === loanId
          ? { ...l, status: newStatus, statusTone: STATUS_CFG[newStatus]?.tone ?? 'neutral', updated }
          : l,
      ),
    );
  }
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {KPI_CFG.map(({ key, label, icon: Icon, iconBg }) => (
          ...
          >
            ...
            <div>
              <p className="text-xs text-text-muted">{label}</p>
              <p className="text-2xl font-bold text-text-primary">{counts[key] ?? 0}</p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="flex items-center gap-0.5 overflow-x-auto border-b border-[#e9e9e9] px-5">
          {LOAN_STATUSES.map((tab) => (
            ...
            >
              ...
              >
                {tab === 'All' ? loans.length : (counts[tab] ?? 0)}
              </span>
            </button>
          ))}
        </div>
