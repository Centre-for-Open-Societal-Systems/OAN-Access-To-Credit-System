# rules.md — The 20 Non-Negotiables
...
- **`NEXT_PUBLIC_` variables must never contain secrets.** Anything prefixed `NEXT_PUBLIC_` is statically embedded into the client-side JavaScript bundle and visible to every user without authentication.
...
- **Every `page.tsx` must export `metadata` or `generateMetadata`.** A page without metadata has no `<title>`, no description, and no OG tags.
...
- **Dark mode variants must be present wherever a light-mode color class exists.** Components without dark mode coverage are visual defects in dark-mode environments.
...
- **Arbitrary Tailwind values repeated more than once across the codebase must be tokenized.**
